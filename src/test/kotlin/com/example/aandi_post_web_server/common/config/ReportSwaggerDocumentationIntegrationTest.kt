        ...
        "report-v2 api docs endpoint는 모든 v2 경로를 함께 포함하고 v1 경로를 섞지 않는다" {
            client().get()
                ...
                .consumeWith { result ->
                    ...
                    body.shouldNotContain("\"/v2/report")
                    body.shouldNotContain("\"/v2/assignments/{assignmentId}/submission-status/me\"")
                    ...
                    body.shouldContain("`deviceOS`, `Authenticate`, `timestamp`, `salt`")
                    body.shouldContain("`/v2/courses/**`, `/v2/admin/courses/**`, `/v2/assignments/**`")
                    body.shouldContain("관리자용 과제 제출 현황 조회")
                    body.shouldContain("projection 이 없으면 미제출")
                    body.shouldContain("\"code\":40301")
                    body.shouldContain("completedAt")
                    body.shouldNotContain("ReportApiEnvelope")
                    body.shouldNotContain("ApiEnvelope(success/data/error/timestamp)")
                    body.shouldNotContain("deprecated alias")
                    body.shouldNotContain("\"deprecated\":true")
                }
        }
