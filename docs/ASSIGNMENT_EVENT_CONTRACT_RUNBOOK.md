...
| MongoDB transaction | 현재 assignment command/copy write는 순차화했지만 transaction manager/unit of work가 없으며 운영 replica-set 지원도 미확인 |
| outbox/relay | 미구현 |
...
6. transaction 안의 현재 assignment MongoDB write를 순차 실행하도록 준비합니다. 생성·수정·복사와 delayed-error cascade/cleanup 삭제의 실제 구독 순차화를 완료했습니다.
...
- [x] cascade/cleanup 삭제 세 경로를 전체 시도·오류 보존 계약과 함께 순차 구독으로 전환했다.
