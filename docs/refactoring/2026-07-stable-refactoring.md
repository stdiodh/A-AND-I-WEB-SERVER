...
| 7A-2. 과제 삭제 write 순차화 | 완료 | cascade·copy cleanup 삭제를 순차 구독하면서 전체 시도와 delayed-error 계약을 유지 |
...
앞 단계에서 제외했던 단건 cascade, 코스 일괄 cascade, 과제 복사 실패 cleanup을 `Flux.concatDelayError`로 전환했습니다. 삭제는 requirements → testCases → deliveries → assignment 순서로 하나씩 구독하지만, 중간 단계가 실패해도 나머지 삭제를 모두 시도한 뒤 오류를 전파합니다.

- 단일 삭제 오류는 원본 오류를 그대로 전파하고 problem event를 발행하지 않음
- 복수 삭제 오류는 모든 삭제 완료 뒤 함께 전파하고 problem event를 발행하지 않음
...
생성·수정·복사의 child write와 cascade/cleanup 삭제는 실제 subscription 기준으로 순차화했습니다. 현재 assignment command의 MongoDB write 순서 준비는 완료했지만 transaction manager, outbox 저장, 운영 replica-set 검증은 아직 없습니다.
